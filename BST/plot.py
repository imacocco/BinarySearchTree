...
def print_averages(Directory,nome_file):
	ordered_tree=np.loadtxt(Directory+"Balanced_tree.txt")
	...
	plt.errorbar(ordered_tree[:,0],ordered_tree[:,1],yerr=ordered_tree[:,2],label="Balanced_tree")
	plt.errorbar(unbalanced_tree[:,0],unbalanced_tree[:,1],yerr=unbalanced_tree[:,2],label="Unbalanced_tree")
	...
	plt.title(nome_file)
	plt.savefig(Directory+nome_file+".png")
	plt.show()
